...
<!DOCTYPE html>
	...
	<body>
		...
		<input type="button" value="주소창 전달" onclick="sendAddress()">
...
<script>
    ...
    function sendAddress() {
        ...
        var extraAddress = document.getElementById('sample2_extraAddress').value;

        // 모든 필드가 입력되었는지 확인
        if (postcode === '' || address === '' || detailAddress === '') {
            alert('참고항목을 제외한 모든 필드를 입력해주세요.');
            return;
        }
        ...
        // 부모 창으로 주소 정보 전송 (부모 창이 window.opener로 참조됨)
        window.opener.receiveAddress(fullAddress,postcode);
