...
<body>
	<div id="wrap">
		...
		<div id="main" class="dflex">
			...
			<div id="mainContent">
				<div class="main_banner">배너
				
				</div>
				<div class="product">
					<c:forEach var="goodsData" items="${goodsList}">
						<a href="goodPage?idx=${goodsData.idx}"> <img alt="상품이미지"
							src="${pageContext.request.contextPath}/resources/images/${goodsData.g_title}">
				...
				<div class="paging">
					<c:if test="${nowpage!=1}">
						<c:choose>
							<c:when test="${nowpage>jumpgingPage}">
								<a href="index?nowpage=${nowpage-jumpgingPage}">◀</a>
							</c:when>
							<c:otherwise>
								<a href="index?nowpage=1">◀</a>
							</c:otherwise>
						</c:choose>
					</c:if>
					<c:forEach var="index" begin="${minpage}" end="${maxpage}" step="1">
						<c:choose>
							<c:when test="${index==nowpage}">
								<a href="index?nowpage=${index}" class="bolder">${index}</a>
							</c:when>
							<c:otherwise>
								<a href="index?nowpage=${index}">${index}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${nowpage!=maxpage}">
						<c:choose>
							...
							<c:otherwise>
								<a href="index?nowpage=${pagingCount}">▶</a>
							</c:otherwise>
						</c:choose>
					</c:if>
				</div>
