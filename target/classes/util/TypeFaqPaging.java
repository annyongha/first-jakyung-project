...
public class TypeFaqPaging {

	// page_product_quantity = 페이지당 보여줄 상품리스트 개수
	// nowpage = 요청된 페이지 인덱스 정보
	// Object dao = 데이터 조회에 사용되는 연결객체
	public static HashMap<String, Object> getPaging(int page_product_quantity, String nowpage, FaqDAO dao,String type) {
		...
		goodsList = dao.type_faq_list(intnowpage, page_product_quantity,type);

		// 총 데이터 수량을 저장
		goodsCount = dao.type_faq_list_Count(type);
